...
All recruiter-target prose validators apply the same bounded NFKC, nested-entity, and percent-decoding safety view before checking contact, URL, path, or credential-shaped material. Localized shortlist and decision-gate renderers translate closed segment and missing-context values into candidate-facing labels; internal enum names never reach the HTML review surface.

The same boundary rejects Unicode control and format characters (including zero-width, bidi, NUL, and newline controls) in diagnostic field names and dossier prose, preventing hidden or injected material from reaching validation output or local HTML. Candidate-facing Markdown also rejects URI schemes with an authority outside the permitted validated LinkedIn sources, including `ftp://`, `ws://`, `gopher://`, and embedded userinfo.
...
Actor-before-event forms (`prepare for a first recruiter interview`, `get ready for my first call with the recruiter`, and `prepare for a first recruiter screen`) and natural Spanish forms (`quiero prepararme/practicar mi primera entrevista o conversación con adquisición de talento`) use the same screen-intake boundary. Technical suffixes such as `recruiting systems`, `recruiting operations`, and `talent acquisition analytics` remain ordinary domain language.

Direct artifact requests are also explicit: `build/create/prepare a recruiter shortlist`, `recruiter target shortlist`, `shortlist of recruiters`, and Spanish `crear una shortlist de reclutadores` or `lista corta de objetivos recruiter` enter the manual `recruiter_target_shortlist` intake. Shortlists of technical questions or platform topics remain ordinary coaching, so the artifact route is reserved for recruiter targets.

Practice wording is bounded as well: `practice/rehearse a recruiter screen or interview`, `have a recruiter screen and want a practice session`, and Spanish `practicar/ensayar un filtro o entrevista con el reclutador` enter artifact-free screen-intake for private preparation. If the wording says the screen was completed, post-screen debrief keeps precedence; technical interview practice without recruiter context remains ordinary coaching. Spanish post-screen forms with `mi llamada` or `mi conversación` and an organizational alias also enter private debrief continuity.
...
filtro”, “siguió con otra persona”, “no fui seleccionado después de la
entrevista”, and “el reclutador eligió a otra persona” keep negative-outcome
precedence and enter the artifact-free private debrief route. They do not
become a next-stage review or ordinary coaching request.
...
In `prefers-contrast: more`, screen-intake and screen-debrief cards and coverage rows use 2px borders and 0.5rem state bands, matching the stronger contrast treatment across the recruiter review family. Compact conversion-outcome and follow-through checkpoint receipts also use 2px rail, step, and marker borders, preserving state legibility without relying on color alone. In forced-colors mode, shortlist and decision-gate rows retain solid, dashed, double, and dotted left-border styles for `advance`, `clarify`, `pause`, and `stop` instead of flattening every decision to a solid bar.

The decision gate, screen-intake bridge, post-screen debrief, and next-stage review now return the same private in-memory `rendered_html` contract whenever a validated artifact exists. Intake failures remain artifact-free; stopped or blocked artifacts still render their localized review surface without IDs, snapshots, contacts, or action tokens. The screen-intake, debrief-intake, full-debrief, and next-stage routers also catch bounded recursion failures from malformed nested in-memory context and recover with `needs_intake` instead of leaking a traceback. The next-stage schema now treats `offer_stage` as terminal, matching runtime recovery and rejecting any schema-only forward transition from that stage.

The five recruiter review surfaces also render the shared identity-free continuity rail from `scripts/recruiter_continuity_rail.py`: choose targets, review the decision, prepare/review the screen, and prepare the next stage. It uses localized candidate-facing labels inside a static `section` (not a navigation landmark), visibly states that it is orientation rather than progress or contact tracking, labels the current item as the current review surface (not a completed milestone), marks only that surface with `aria-current="location"`, remains non-interactive, collapses to one column below 420px, and keeps the same offline, print, forced-colors, and responsive boundary without adding candidate data or external actions. Decision-gate, screen-intake, and post-screen debrief footers are atomic in print alongside their cards. In forced-colors mode the shortlist priority panel resets to `Canvas`/`CanvasText` with an explicit `CanvasText` border so its color-mix accent cannot reduce readability.
