...
10. **Ask the essence question.** Confirm that the renderer summary asks only the first pending authorization question from the validated dossier, when one exists; on an explicit positive answer, inspect only that named section immediately. Do not ask it again, do not write `authorized_for_session` or any session identifier, and do not add intake questions that cannot change a recommendation or copy decision.
