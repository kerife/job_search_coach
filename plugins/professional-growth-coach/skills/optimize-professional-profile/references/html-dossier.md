...
5. **Validate and repair once.** Run `validate_executive_career_dossier_v2.py` against the temporary JSON. If it exits nonzero, use only its path-based errors to repair once; never expose the rejected JSON or error values. The v1/v2 dossier loaders reject oversized integer values at the same private JSON boundary as malformed JSON, invalid UTF-8, duplicate keys, unsafe nesting, symlinks, and oversized files. Their validator and renderer CLIs fail with the existing opaque contract: return code `3`, empty stdout, no traceback, and no reflection of raw values, paths, or identifiers. Valid dossier inputs keep their existing validation and render output.
...
8. **Prove the artifact succeeded.** Verify renderer exit 0, an existing regular non-symlink file, mode 600, `artifact_type=text/html`, the expected locale, and an absolute receipt path that resolves to the same output. Private JSON inputs and market-learning outputs use descriptor-anchored paths, reject parent/leaf symlinks and hardlinked targets, publish atomically without overwrite, and remove temporary files after a failed write.
