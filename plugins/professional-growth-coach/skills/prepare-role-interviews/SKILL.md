...
The private `private-recruiter-screen-debrief-v1` bridge is the bounded follow-up after `screen_attended`. It may expose `manual_prepare_next_stage_review` only when the linked target-specific intake is still `ready`, all three coverage topics are `discussed`, and no unknown topics remain. Otherwise keep `collect_debrief_context` or terminal `record_stop_decision`; this is a manual review cue, not a preparation run, answer store, message, calendar action, or outcome prediction.
