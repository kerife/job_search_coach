...
Preserve current employment by default (`preserve_current_employment_by_default`). Scores compare research and positioning options; they never instruct a person to resign, quit, leave an employer, reduce hours, or create a voluntary gap. Staying and growing in the current role is a valid option (`staying_and_growing_is_valid`), as is `do_nothing_now`; any explicit separation analysis must set `no_resignation_recommendation=true` and remain a neutral scenario matrix.
...
3. If salary, demand, or a ranking affects the choice, request `research-professional-market` with roles and distinct geography/employment scenarios. Do not independently assert current salary, demand, or a highest-paying ranking.
