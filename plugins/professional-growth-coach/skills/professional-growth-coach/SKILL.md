...
Except for explicit private recruiter-practice or private recruiter-reply triage requests, inbound recruiter contact/invitations (`messaged`, `emailed`, `reached out`, passive `was contacted by`, calendar links, `me escribió`, `me contactó`, `me pidió disponibilidad`; aliases `recruiting`, `talent acquisition`, `adquisición de talento`, `sourcer`, `headhunter`, `reclutamiento`) and send/confirm/accept/schedule/book/message/reply requests route first to `optimize-professional-profile` and require one `recruiter_reply_triage` row before any draft. Preparation invitations use screen-intake; action-shaped ones preserve auth; use reply triage. Explicit private practice wins even with those signals or debug, raw, or internal-row requests. Explicit private recruiter-reply triage also wins even with those signals or debug, raw, or internal-row requests. Neither emits router rows or a module-execution packet; see [routing.md](references/routing.md).
