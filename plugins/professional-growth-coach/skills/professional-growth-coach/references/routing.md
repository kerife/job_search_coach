...
Localized first-interview preparation also recognizes `adquisición de talento`,
`reclutamiento`, `sourcer`, and `headhunter` in forms such as `tengo una
entrevista inicial`, `prepararme para una primera entrevista`, and `prepararme
para hablar con`. These requests enter artifact-free `recruiter_target_screen_intake`
with `collect_screen_intake`; networking or connecting with those aliases stays
...
Spanish post-screen practice wording (`practicar`, `repasar`, or `prepararme` after `hablar`, a recruiter call, or a recruiter interview) is debrief continuity when recruiter context is present, returning artifact-free `private_recruiter_screen_debrief` with `collect_debrief_context`. First-call practice without a prior conversation remains screen-intake; the same wording with a client or other non-recruiter remains ordinary.
...
The same confirmation boundary is localized: `El reclutador me pidió confirmar la entrevista`, `La reclutadora quiere que acepte la entrevista`, and `Reclutamiento me pidió confirmar la entrevista` require private reply triage and authorization. Passive `screen was/got canceled`, `La entrevista con el reclutador fue cancelada`, `La entrevista con el reclutador fue reprogramada`, or `I was not selected after the recruiter screen` wording follows screen-intake or private debrief respectively; technical outcomes without recruiter context stay ordinary.

The same invitation precedence applies when the actor is an organizational alias (`recruiting`, `recruitment`, `talent acquisition`, `adquisición de talento`, `talent partners`, `sourcer`, `headhunter`, or `reclutamiento`): alias-plus-invitation language enters screen-intake for preparation, and enters reply triage with authorization when the request asks to reply, accept, confirm, or follow up. A technical interview invitation without recruiter context stays ordinary.
...
Inbound scheduling variants from an organizational alias (`Adquisición de talento compartió los horarios`, `pidió escoger un horario`, or `quiere agendar una llamada/entrevista`, plus their English equivalents) also enter `private_recruiter_reply_triage` with `next_action=collect_recruiter_reply_triage_context`, `authorization_required=true`, and `artifact=None`; never accept, propose, or create a calendar event from this context.

Connection requests naming an alias target (`I want to connect with a sourcer`, `Please connect me with a sourcer`, or `Quiero conectar con un sourcer`) enter the manual `recruiter_target_shortlist` intake with `authorization_required=true`. The shortlist collects target context only; it does not send a connection request. Analysis-only `network with` language remains the non-authorizing shortlist boundary.
...
Direct shortlist requests (`build/create/prepare a recruiter shortlist`, `recruiter target shortlist`, `shortlist of recruiters`, `crear una shortlist de reclutadores`, or `lista corta de objetivos recruiter`) enter `recruiter_target_shortlist` with `case_state=needs_intake` until three to six manual targets and the bounded network plan are supplied. A shortlist of technical interview questions or platform topics remains ordinary professional-growth routing; the recruiter actor is required for this branch.

Practice requests (`practice/rehearse a recruiter screen or interview`, `have a recruiter screen and want a practice session`, or Spanish `practicar/ensayar un filtro o entrevista con el reclutador`) enter artifact-free `recruiter_target_screen_intake` with `collect_screen_intake` and no authorization. Completed-screen wording keeps post-screen debrief precedence, while technical interview practice without recruiter context remains ordinary. Spanish post-screen practice may use `mi llamada` or `mi conversación` with an organizational alias and still enters `private_recruiter_screen_debrief` with `collect_debrief_context`.
