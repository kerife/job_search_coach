...
Natural post-screen requests use precedence `debrief > next_stage > shortlist > ordinary`. A recruiter/screen context combined with `debrief`, review language, or explicit completed-screen wording routes artifact-free to `private_recruiter_screen_debrief` with `selected_module=track-career-outcomes` and `next_action=collect_debrief_context`; an explicit next-stage phrase routes to `private_recruiter_next_stage_review` with `selected_module=prepare-role-interviews` and the same context-collection boundary. The classifier requires recruiter or screen/interview context, so generic technical-interview preparation remains ordinary coaching and recruiter networking remains the shortlist route. Contact, send, reply, connect, schedule, and similar wording still sets `authorization_required=true` without performing an action.
