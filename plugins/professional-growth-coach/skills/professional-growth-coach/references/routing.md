...
After the shortlist is validated, `route_recruiter_decision_gate` may build `recruiter-target-decision-gate-v1`. It binds the full shortlist snapshot, reconciles `advance|clarify|pause|stop` counts, and returns `collect_screen_context` until a bounded vacancy summary and one confirmed fact are supplied. With those fields it returns `prepare_role_interviews_review`, still `draft_only=true`, `manual_review_required=true`, `no_message_action=true`, and `no_calendar_action=true`; it never starts preparation or performs an external action.
