#!/usr/bin/env python3
"""Run static checks for the Job Search Coach plugin."""
...
FINAL_CASE_REQUIRED_PATTERNS = {
    "senior-technical": (
        ...
        (r"selected_module:\s*research-target-job-market", "market-first route"),
    ),
    "non-technical-transition": (
        ...
        (r"selected_module:\s*research-target-job-market", "evidence-first market route"),
    ),
    "junior": (
        ...
        (r"selected_module:\s*optimize-linkedin-career", "LinkedIn intake route"),
    ),
    "imminent-interview": (
...
EXECUTIVE_DOSSIER_PRESSURE_SOURCE_PATHS = (
    ...
    "plugins/job-search-coach/skills/job-search-coach/references/evidence-and-safety.md",
    "plugins/job-search-coach/skills/job-search-coach/references/routing.md",
    ...
    "plugins/job-search-coach/skills/optimize-linkedin-career/references/profile-audit.md",
)
...
def validate_executive_dossier_pressure_summary(
    ...
    if not isinstance(corpus_cases, list):
        ...
        for case in corpus_cases:
            ...
            if case.get("entrypoint") != "skills/optimize-linkedin-career/SKILL.md":
    ...
    if not isinstance(control, dict):
        ...
        if control.get("entrypoint") != "skills/optimize-linkedin-career/SKILL.md":
        ...
        if not isinstance(snapshot, dict):
            ...
            if isinstance(source_commit, str) and isinstance(plugin_tree, str):
                result = subprocess.run(
                    ["git", "rev-parse", f"{source_commit}:plugins/job-search-coach"],
                    cwd=repository_root,
                    check=False,
                    capture_output=True,
                    text=True,
                ...
                manifest_result = subprocess.run(
                    [
                        "git",
                        "show",
                        f"{source_commit}:plugins/job-search-coach/.codex-plugin/plugin.json",
                    ],
                    cwd=repository_root,
                    check=False,
                    capture_output=True,
...
def validate_eval_provenance(artifact: object, repo_root: Path) -> list[str]:
    ...
    resolved_tree = _git_output(
        repo_root, "rev-parse", f"{source_commit}:plugins/job-search-coach"
    )
...
def validate_coach_executive_review_quality(raw_output: str) -> list[str]:
    ...
    allowed_workstreams = {
        "linkedin_positioning": "optimize-linkedin-career",
        "application_packet": "optimize-job-search-assets",
        "market_targeting": "research-target-job-market",
        "interview_prep": "prepare-role-interviews",
        "outcome_tracking": "track-job-search-outcomes",
    }
...
def validate_linkedin_target_role_positioning_board_quality(raw_output: str) -> list[str]:
    ...
    if board_lines:
        ...
        expected = {
            ...
            "next_research_module": "research-target-job-market",
            "no_external_action": "true",
            "draft_only": "true",
        }
    ...
    for line_number, line in enumerate(lane_lines, start=1):
        ...
        if parsed.get("market_research_gate") != "research-target-job-market_before_pay_or_demand_claim":
...
def validate_linkedin_target_vacancy_alignment_card_quality(raw_output: str) -> list[str]:
    ...
    expected_values = {
        ...
        "research_handoff": "research-target-job-market",
...
def validate_high_value_role_opportunity_matrix(raw_output: str) -> list[str]:
    ...
    for line_number, line in enumerate(research_plan_lines, start=1):
        ...
        if parsed.get("research_module") != "research-target-job-market":
            errors.append(f"market_research_execution_plan {line_number} must route to research-target-job-market")
    ...
    for line_number, line in enumerate(highest_pay_audit_lines, start=1):
        ...
        if "research-target-job-market" not in parsed.get("next_research_action", ""):
            errors.append(f"highest_pay_claim_audit {line_number} must route to research-target-job-market")
        if parsed.get("no_salary_claim") != "true" or parsed.get("draft_only") != "true":
...
def main() -> int:
    ...
    if not manifest_path.is_file():
        ...
        if manifest.get("name") != "job-search-coach":
    ...
    root_skill = (SKILLS_ROOT / "job-search-coach" / "SKILL.md").read_text(encoding="utf-8")
    routing = (
