...
- `optimize-linkedin-career`: LinkedIn/CV positioning, profile drafts, or a profile conflict.
- `discover-high-value-career-paths`: role transition or high-compensation direction before a concrete market question.
- `research-target-job-market`: current demand, compensation, role requirements, or a target vacancy.
- `optimize-job-search-assets`: CV, cover letter, portfolio, or ATS assets without a LinkedIn-specific need.
...
- `track-job-search-outcomes`: a 14/30/60/90-day results review.
...
- Start with evidence repair or `optimize-linkedin-career` when visible profile facts conflict with CV facts.
- Use `research-target-job-market` before `discover-high-value-career-paths` when current demand, compensation, geography, or role requirements are needed.
- Use `optimize-job-search-assets` before `prepare-role-interviews` when the vacancy-specific fact matrix is missing.
...
- Use `track-job-search-outcomes` only after dated application, response, interview, or offer records are available for one isolated candidate.
