...
description: Use when routing a self-service or coach-mode job-search case, resolving evidence conflicts, isolating candidate data, selecting a career module, or checking consent and authorization before external career actions.
---
...
When `selected_module=optimize-linkedin-career` and `case_state=ready`, defer presentation to the LinkedIn skill and select one branch:
...
Except for explicit private recruiter-practice or private recruiter-reply triage requests, inbound recruiter replies, recruiter screen invitations, proposed meeting times, and requests to send, confirm, accept, schedule, book, message, reply, or create a calendar item route first to `optimize-linkedin-career` and require one `recruiter_reply_triage` row before any draft response. Explicit private practice wins even with those signals or debug, raw, or internal-row requests. Explicit private recruiter-reply triage also wins even with those signals or debug, raw, or internal-row requests. Neither emits recruiter triage, router rows, or a module-execution packet; use [routing.md](references/routing.md) for the non-private authorization and time-handling rules.
...
For a multi-module case outside the normal LinkedIn artifact chat, also include one `coach_case_brief` after the router contract. This is the executive case-management layer that turns module routing into a practical first-interview plan. It must be one semicolon-delimited `inferred:` row with `candidate_id`, `case_goal`, `coach_verdict`, `evidence_strength`, `primary_bottleneck`, `module_sequence`, `handoff_ready`, `first_interview_strategy`, `weekly_commitment`, `success_signal`, `stop_condition`, `privacy_boundary`, and `causality_boundary=descriptive_only_no_guaranteed_outcome`. Use it to connect LinkedIn positioning, application assets, target-market research, interview preparation, learning gaps, and outcome tracking without claiming guaranteed results or causal lift.

Also include one `coach_executive_review` for multi-module work outside the normal LinkedIn artifact chat. This is the candidate-facing decision layer, not a motivational summary. It must be one semicolon-delimited `inferred:` row with `candidate_id`, `diagnosis`, `decision`, `decision_rationale`, `priority_order`, `tradeoffs`, `risk_register`, `seven_day_plan`, `defer_until`, `first_interview_path`, `measurement_plan`, `leading_indicators`, `outcome_signals`, `privacy_boundary`, `authorization_gate`, and `causality_boundary=descriptive_only_no_guaranteed_outcome`. Use prose—not snake_case tokens—for `diagnosis`, `decision`, `decision_rationale`, `tradeoffs`, `risk_register`, `seven_day_plan`, `first_interview_path`, and `measurement_plan`. Use it to tell the candidate what is blocking progress, what to do first, why, what to defer, what could go wrong, what to complete in seven days, which leading indicators to track before outcomes, and how to measure the next signal without promising interviews, offers, faster hiring, compensation, or causal lift.
