---
name: research-target-job-market
...
Collect dated market evidence for a supplied target role or comparison list. This skill produces research inputs for `discover-high-value-career-paths`; it must not choose the candidate's career path and does not recommend or rank career paths.
...
If no comparable compensation evidence exists, leave `range=unknown`; preserve each source as context rather than manufacturing a cross-market range. Return the brief to `discover-high-value-career-paths`, which may combine it with candidate evidence and make only its own conditional path analysis.
