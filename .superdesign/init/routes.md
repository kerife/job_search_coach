...
- **Recruiter target shortlist:** a private three-to-six-target review batch; explicit network intent runs builder → validator → renderer and returns a validated artifact plus private HTML, while missing context yields one intake question. First-conversation wording (`initial interview`, `first call`, `entrevista inicial`, `primera llamada`) and defined articles, including `I have an interview with the recruiter` / `tengo una entrevista con la reclutadora`, route to the same intake. Explicit `never`/`nunca` non-completion wording, including `never went`, `didn't go`, `never went through`, `never spoke`, named future weekday/month dates, invitations, missed/skipped/canceled/rescheduled screens, `nunca fui`, `no fui`, `no me presenté`, `no hablé`, and `nunca pasé por`, routes to screen preparation instead of post-screen debrief, while `had no trouble/questions` and a completed screen followed by `not yet ready for the next stage` remain post-screen language. Future dates are scoped to the recruiter event, including direct weekday, `next Monday`, `in two days`, and `tomorrow` forms, so unrelated appointments cannot reopen pre-screen intake; an invitation to a later stage after completion stays next-stage review. Action synonyms such as `respond`, `write back`, `ping`, `DM`, `email`, `contestar`, `respóndele`, `escríbele`, `enviar`, and `mandar` preserve the authorization requirement on fallback routes. The rail labels the current item as the current review surface with `aria-current="location"` without implying completed progress. Non-advance cards also show a localized contact-boundary explanation derived from the closed reason enum (for example, missing context or confidentiality risk); the internal enum never appears in client-facing HTML, and the explanation is preserved in high-contrast, forced-colors, and print output.
...
`emailed`, `reached out`, schedule/choose-a-time wording, calendar links or proposed times,
