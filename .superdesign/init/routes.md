...
- **Recruiter target shortlist:** a private three-to-six-target review batch; explicit network intent runs builder → validator → renderer and returns a validated artifact plus private HTML, while missing context yields one intake question. First-conversation wording (`initial interview`, `first call`, `entrevista inicial`, `primera llamada`) and defined articles route to the same intake. Explicit `never`/`nunca` non-completion wording, including `never went`, `didn't go`, `nunca fui`, `no fui`, `no me presenté`, and `nunca pasé por`, routes to screen preparation instead of post-screen debrief, while `had no trouble/questions` remains completed-screen language.
